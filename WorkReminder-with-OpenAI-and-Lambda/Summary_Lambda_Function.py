"""
-*- coding: utf-8 -*-
@File  : Summary_Lambda_Function.py
@author: mingyu110
@Description : 
@Time  : 2024/07/01
"""
    ...
    try:
        ...
        try:
            with connection.cursor() as cursor:
                # Query to fetch work task details
                sql = "SELECT task_name, time FROM taskreminder WHERE chatid = %s"
                cursor.execute(sql, (chat_id,))
                results = cursor.fetchall()

            # Create the summary message
            if results:
                summary_message = "Here is a summary of your work tasks:\n"
                for row in results:
                    summary_message += f"{row['task_name']} at {row['time']}\n"
            else:
                summary_message = "You have no work tasks saved."
